...
for (i in countyFiles){
  ...
  # Atlantic has slightly different columns than the other counties:
  if (coName = "atlantic"){
    homes <- read_csv(file.path("~", "inputs", "raw_county", 
                       ...
                       col_types = cols( 
                         ...
                         "Sale Date" = col_date(format = "%Y-%m-%d"), 
                         "Book" = col_character(), 
                         "Page" = col_character(), 
    ...
    # Match column names and number of columns to other counties:
    homes <- homes %>%
      rename(`Deed Date` = `Sale Date`) %>%
      mutate( `SR1A #` = NA) %>%
      relocate(`SR1A #`, .after = `Yr. Built`)
  }
  # read other counties:
  homes <- read_csv(file.path("~", "inputs", "raw_county", 
                               as.character(i)),
